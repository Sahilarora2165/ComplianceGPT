...
def _scrape_portal(portal: dict, hash_db: dict) -> list[dict]:
    ...
    try:
        resp = requests.get(
            portal["url"], headers=HEADERS, timeout=REQUEST_TIMEOUT
        )
        resp.raise_for_status()
        ...
        for row in rows[:20]:
            ...
            # FIX 1: re.sub now uses module-level import (not re-imported per loop)
...
def _ingest_new_docs(new_docs: list[dict]) -> None:
    ...
    """
    # FIX 2: correct import path — ingest.py lives at backend/core/ingest.py
    try:
        from core.ingest import ingest_pdf
    except ImportError as e:
        print(f"  ⚠️  Could not import ingest_pdf from core.ingest: {e} — skipping auto-ingest")
        return
    ...
    for doc in new_docs:
        ...
        if doc["source"] == "simulated":
            # Placeholder text file for demo — real ingest needs an actual PDF
...
def run_monitoring_agent(
    ...
    Returns:
        List of new-document dicts (regulator, title, url, filename,
        priority, summary, source).
    """
    print("\n" + "═" * 60)
